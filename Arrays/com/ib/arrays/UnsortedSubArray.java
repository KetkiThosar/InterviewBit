...
public class UnsortedSubArray {
	...
	static public ArrayList<Integer> subUnsort(ArrayList<Integer> A) {
		...
		/*This solutions creates 3 arrays.one for unsorted array and each for before unsorted part and after
		 unsorted part such that max(beforeUnsorted)<= min(unsorted) && max(unsorted)<=min(afterSorted)*/
		...
		// iterate from 0 to size-2.If next element of array is greater than first one or if it is less than unSortMax it is consider to be unsorted.
		for (int i = 0; i < n - 1; i++) {
			current = A.get(i);
			next = A.get(i + 1);
		...
		// if last element is less than unsorted array add it to unsortedList and update max and min
		...
		// create beforeUnsort list and add elements from 0 till startIndex.calculate its max value.
		...
		// create afterUnsort list and add elements from endIndex+1 till size of original array.Calulate its min value.
		...
		// if sortMax is greater than the unSortMin then add elements from beforeSortList to unSortArray till unSortMin is greater.
		// i.e shift start index to left till the elements in left part are smaller than min value of unsorted array.
